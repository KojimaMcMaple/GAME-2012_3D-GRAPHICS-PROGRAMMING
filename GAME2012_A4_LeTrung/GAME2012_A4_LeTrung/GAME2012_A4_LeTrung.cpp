...
///////////////////////////////////////////////////////////////////////
//
// GAME2012_A4_LeTrung.cpp by Your Name (C) 2020 All Rights Reserved.
//
...
// Our bitflags. 4 bytes for up to 32 keys.
...
PointLight pLight(glm::vec3(15.0f, 1.0f, -5.0f),	// Position.
	1.0f, 0.7f, 1.8f,				// Constant, Linear, Exponent.
	glm::vec3(0.0f, 1.0f, 1.0f),	// Diffuse colour.
	30.0f);						// Diffuse strength.
...
void init(void)
{
	...
	unsigned char* image = stbi_load("GAME2012_A4_LeTrung_LudensTexture.jpg", &width, &height, &bitDepth, 0);
	if (!image) cout << "Unable to load file!" << endl;

	glGenTextures(1, &ludens_texture);
	glBindTexture(GL_TEXTURE_2D, ludens_texture);
...
void display(void)
{
	...
	glBindTexture(GL_TEXTURE_2D, ludens_texture);
	gGrid.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo, &normals_vbo, program);
	transformObject(glm::vec3(10.0f, 10.0f, 1.0f), X_AXIS, -90.0f, glm::vec3(0.0f, 0.0f, 0.0f));
	//transformObject(glm::vec3(10.0f, 10.0f, 1.0f), X_AXIS, 0.0f, glm::vec3(0.0f, 0.0f, 0.0f));
	glDrawElements(GL_TRIANGLES, gGrid.NumIndices(), GL_UNSIGNED_SHORT, 0);
...
void parseKeys()
{
	...
	if (keys & KEY_ALT_FORWARD)
		pLight.position -= ground_front_vec * MOVESPEED;
	else if (keys & KEY_ALT_BACKWARD)
		pLight.position += ground_front_vec * MOVESPEED;
	if (keys & KEY_ALT_LEFT)
		pLight.position -= rightVec * MOVESPEED;
	else if (keys & KEY_ALT_RIGHT)
		pLight.position += rightVec * MOVESPEED;
}
...
void clean()
{
	...
	glDeleteTextures(1, &ludens_texture);
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("GAME2012_A4_LeTrung");
